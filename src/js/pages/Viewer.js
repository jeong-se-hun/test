import { createElement, fetchData, getPayload, isAdult } from '../app.js';
...
const Viewer = async params => {
  ...
  const { title, cover, adult } = selectedData;
  ...
  <div class="viewer">
    ...
    <div class="viewer__webtoon" data-theme="${personalSettings.theme}" style="zoom: ${personalSettings.zoom + '%'}">
      ${bottomNavItems.map(() => `<img src="${isAdult(cover, adult)}" alt="${title} 컷씬" />`).join('')}
    </div>
